...
"syn match cParents "[(\)]"
"syn match cAccolade "[\{\}]"
"syn match cConst "\v[A-Z_][A-Z0-9_]{1,}"
syn match cSymbols "\v(\s((\&\&)|(\|\|)|((\=|\+|\-|\*|\/)?\=)|\+|\-|\*|\/|\<\=?|\>\=?|\?)\s)"
syn match cSymbols "\v(\+\+)|(\-\-)|(\s\%\s)"
syn match cArrows "\v(\-\>)|\."
syn match cPointer "\v(\*\*?)|!|\&|\~|\||\^|(\>\>)|(\<\<)"

" C Functions
syn keyword cFunctions strlen strtol malloc calloc realloc free atexit exit abort getenv qsort rand srand time itoa ltoa ultoa printf fprintf sprintf scanf fscanf putchar puts fputc gets fgets fopen fclose snprintf fwrite fread tmpfile tmpnam remove rename clock strcpy strncpy strcat strncat strcmp strncmp strchr strrchr strstr strspn strcspn strpbrk strtok tolower toupper sin cos tan asin acos atan exp log log10 pow sqrt fabs floor ceil getc getchar fread feof fgetc rewind getpid getppid getuid getgid fork perror execv execve execl execle execlp system sleep usleep write read ftell fseek freopen close opendir closedir readdir fgetpos fsetpos rewinddir seekdir telldir open
"GTK functions
syn keyword cFunctions gtk_init gtk_window_new g_signal_connect gtk_widget_show gtk_main gtk_main_quit gtk_window_set_position gtk_window_get_position gtk_window_get_default_size gtk_window_set_default_size gtk_window_set_title gtk_label_set_use_markup gtk_container_add gtk_widget_show_all gtk_widget_destroy gtk_label_new gtk_window_get_title gtk_window_move gtk_window_set_resizable gtk_window_get_resizable gtk_window_set_gravity gtk_window_get_gravity gtk_window_set_focus gtk_window_get_focus gtk_window_set_default gtk_window_iconify gtk_window_deiconify gtk_window_maximize gtk_window_unmaximize gtk_window_set_decorated gtk_window_get_decorated gtk_window_resize gtk_label_set_justify gtk_label_get_justify gtk_label_set_text gtk_label_set_label gtk_label_get_text gtk_label_get_label gtk_label_get_use_markup g_locale_to_utf8 g_free gtk_button_new gtk_button_new_with_label gtk_button_new_with_mnemonic gtk_button_new_from_stock gtk_table_new gtk_table_attach 
"Pthread functions
syn keyword cFunctions pthread_create pthread_exit pthread_join pthread_mutex_destroy pthread_mutex_lock pthread_mutex_unlock pthread_cond_wait pthread_cond_wait pthread_mutex_init pthread_cond_init pthread_cond_destroy pthread_detach
"Sockets functions
syn keyword cFunctions send sendto recv recvto socket gethostbyname htons connect htonl listen accept bind 
"Ncurses functions
syn keyword cFunctions wrefresh newwin delwin wmove wprintw werase keypad wgetch getch start_color init_pair wbkgd cbreak waddch scrollok mvwaddch addch wechochar echochar waddstr addstr attron wattron wborder whline mvwprintw mvaddstr deleteln mvwaddstr subwin wresize clear wclear erase werase mvwin box printw memmove initscr noecho curs_set refresh mvprintw endwin

" C types
syn keyword cType	GtkWidget GTK_WINDOW GTK_CONTAINER GTK_WIDGET GTK_TABLE G_OBJECT G_CALLBACK GTK_LABEL gint gchar guint GtkContainer GtkButton GtkObject
syn keyword cType socklen_t  
syn keyword cType pthread_t pthread_mutex_t pthread_cond_t

" C constants

syn keyword cConstant PTHREAD_COND_INITIALIZER PTHREAD_MUTEX_INITIALIZER TRUE FALSE
...
"hi def link cParents Paren
hi def link cBracket Bracket
"hi def link cAccolade Accolade
"hi def link cConst Constant
hi def link cPointer Conditional
hi def link cSymbols Operator
hi def link cArrows Structure
...
hi def link cConstant	Exception
